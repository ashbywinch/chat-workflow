...
import chat_workflow.conversation_orchestrator
...
class TestLLMInteraction(unittest.TestCase):
    def _create_orchestrator(self, max_turns=10):
        return chat_workflow.conversation_orchestrator.ConversationOrchestrator(
    ...
    def test_call_llm_passes_correct_parameters(self, mock_get_client):
        mock_client = tests.conftest.MockInstructorClient()
        expected_action = chat_workflow.ConversationAction[EvaluationCriteria](action="continue", message="Test")
        ...
        self.assertEqual(response_model, chat_workflow.ConversationAction[EvaluationCriteria])
    ...
    def test_conversation_success_completion(self, mock_get_client):
        success_action = chat_workflow.ConversationAction[EvaluationCriteria](
            action="success", result=tests.conftest.make_valid_criteria()
        )
    ...
    def test_conversation_turn_limit_enforcement(self, mock_get_client):
        continue_action = chat_workflow.ConversationAction[EvaluationCriteria](
    ...
    def test_conversation_failure_action_termination(self, mock_get_client):
        failure_action = chat_workflow.ConversationAction[EvaluationCriteria](
        ...
        with self.assertRaises(chat_workflow.exceptions.ConversationFailedError) as context:
