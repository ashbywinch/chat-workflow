"""Tests for prompt_core.llm_interaction.get_client — provider resolution."""
...
class TestGetClient(unittest.TestCase):
    ...
    def test_unsupported_provider_raises_error(self):
        ...
        from prompt_core.exceptions import ProviderNotSupportedError
    ...
    def test_missing_api_key_raises_error(self):
        from prompt_core.llm_interaction import get_client
        from prompt_core.exceptions import APIKeyError
    ...
    def test_provider_case_insensitive(self):
        ...
        with patch.dict("os.environ", {"OPENAI_API_KEY": "sk-test"}, clear=True):
            with patch("prompt_core.llm_interaction.instructor") as mock_instructor:
        ...
        for provider_name, env_var in providers.items():
            with self.subTest(provider=provider_name):
                with patch.dict("os.environ", {env_var: "test-key"}, clear=True):
                    with patch(
                        "prompt_core.llm_interaction.instructor"
                    ) as mock_instructor:
                        mock_instructor.from_litellm.return_value = "client"
...
class TestListAvailableProviders(unittest.TestCase):
    ...
    def test_all_unset(self):
        from prompt_core.llm_interaction import list_available_providers
